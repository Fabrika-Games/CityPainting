...
public class UIManager : MonoBehaviour
{
    ...
    {
        public ScreenUI.Id id;
    }


    private class QueuedScreenPush : QueuedScreen
    {
        public ScreenUI.Data data;
    ...
    public delegate void PushedDelegate(ScreenUI screen);

    public delegate void PoppedDelegate(ScreenUI.Id id);
    ...
    private Dictionary<string, ScreenUI> _cache;
    private Queue<QueuedScreen> _queue;
    private List<ScreenUI> _stack;
    private HashSet<ScreenUI.Id> _stackIdSet;
    ...
    {
        get { return _rootCanvasScalar.referenceResolution; }
    }
    ...
    private void Awake()
    {
        ...
        _cache = new Dictionary<string, ScreenUI>();
        _queue = new Queue<QueuedScreen>();
        _stack = new List<ScreenUI>();
        _state = State.Ready;
    ...
    /// </summary>
    public void QueuePush(ScreenUI.Id id)
    {
        if (id == null)
        {
            return;
        ...
        QueuePush(id, null, "Screen_" + id, null);
    }

    public void QueuePush(ScreenUI.Id id, ScreenUI.Data data)
    {
        QueuePush(id, data, "Screen_" + id, null);
    }

    public void QueuePush(ScreenUI.Id id, PushedDelegate callback = null)
    {
        QueuePush(id, null, "Screen_" + id, callback);
    }

    public void QueuePush(ScreenUI.Id id, ScreenUI.Data data, PushedDelegate callback = null)
    {
        QueuePush(id, data, "Screen_" + id, callback);
    }

    public void QueuePush(ScreenUI.Id id, ScreenUI.Data data, string prefabName = "null", PushedDelegate callback = null)
    {
    ...
    /// </summary>
    public void QueuePopTo(ScreenUI.Id id, bool include, PoppedDelegate callback = null)
    {
#if PRINT_QUEUE
    ...
    public void QueuePop(PoppedDelegate callback = null)
    {
        ...
        ScreenUI topScreen = GetTopScreen();
        if (topScreen == null)
            return;
    ...
    public ScreenUI GetScreen(ScreenUI.Id id)
    {
    ...
    public T GetScreen<T>(ScreenUI.Id id) where T : ScreenUI
    {
        ScreenUI screen = GetScreen(id);
        return (T) screen;
    }
    ...
    private void ExecuteNextQueueItem()
    {
        ...
        if (queued is QueuedScreenPush)
        {
            ...
            else
            {
                ...
                ScreenUI prefab = Resources.Load<ScreenUI>(path);
        ...
        else
        {
            ...
            ScreenUI screenToPop = GetTopScreen();
    ...
    private void UpdateSortOrderOverrides()
    {
        ...
        {
            var screen = this.rootCanvas.transform.GetChild(i).GetComponent<ScreenUI>();
            if (screen != null)
            {
    ...
    /// </summary>
    //private bool ScreenWillExist (Screen.Id id)
    //{
    //    return false;
    ...
    private void DebugPrintCache(string optionalEventMsg)
    {
        ...
        foreach (KeyValuePair<string, ScreenUI> cached in _cache)
        {
    ...
    private void HandlePushFinished(ScreenUI screen)
    {
    ...
    private void HandlePopFinished(ScreenUI screen)
    {
    ...
    public void AllScreensPop()
    {
        for (int i = 0; i < _stack.Count; i++)
        {
            QueuePopTo(_stack[i].id, true);
        }
    }
}