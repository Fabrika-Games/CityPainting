...
internal struct PoolData
{
    ...
    [SerializeField] private bool _nonLazy;
...
public class M_Pool : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (namesGroups.Count() > 0)
            throw new Exception(
                $"Pool Manager already contains pool with name \"{namesGroups.First().Select(g => g).First()}\"");
    ...
    public static M_Pool II;

    public static M_Pool I
    {
        get
        {
            if (II == null)
            {
                II = GameObject.Find("M_Pool").GetComponent<M_Pool>();
            }
...
internal class PoolManagerInitializer
{
    ...
    {
        var files = Directory.GetFiles("Assets", "M_Pool.cs", SearchOption.AllDirectories);
        var poolManagerFile =
            files.FirstOrDefault(file => file.Replace('\\', '/').EndsWith("Redcode/Pools/Scripts/M_Pool.cs"));
